...
import { Calculator, LogOut, Menu, X, Settings } from 'lucide-react';
...
export const AccountantLayout: React.FC = () => {
  ...
  const navItems = [
    { path: '/accountant/dashboard', name: 'Calculator', icon: Calculator },
  ];

  // Dynamic title for header
  const getPageTitle = () => {
    if (location.pathname.startsWith('/accountant/groups/')) {
      return 'Group Details';
    }
    return navItems.find((item) => item.path === location.pathname)?.name || 'Accountant Panel';
  };
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shadow-sm shrink-0">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="text-right">
              ...
              <p className="text-xs font-semibold text-indigo-600 bg-indigo-50 px-2.5 py-0.5 rounded-full inline-block">
                Accountant
              </p>
            </div>
